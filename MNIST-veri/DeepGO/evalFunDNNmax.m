function [z, imageTemp, l_i,u_i] = evalFunDNNmax(pixelOptIndex,x,imageTemp,verbose)
...
elseif pixelOptIndex > 1
    ...
    z1 = evalFunDNNmax(pixelOptIndex,x1,imageTemp,verbose);
    z2 = evalFunDNNmax(pixelOptIndex,x2,imageTemp,verbose);
    z_Allsorted = [x1,z1;x2,z2];
    y_2 = calculate_y_i(z_Allsorted,K);
    w_y_2 = evalFunDNNmax(pixelOptIndex,y_2,imageTemp,verbose);
    ...
    while(k < maxIter && u_i-l_i > bounderror )
        ...
        w_y_i = evalFunDNNmax(pixelOptIndex,y_i,imageTemp,verbose);
        y_iplus_1 =  calculate_y_i(z_Allsorted(z_starIndex+1:z_starIndex+2,:),K);
        w_y_iplus_1 = evalFunDNNmax(pixelOptIndex,y_iplus_1,imageTemp,verbose);
